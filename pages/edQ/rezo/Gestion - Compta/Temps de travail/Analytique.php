...
$sql = "SELECT * FROM (
	...
	, IF(pv.value IS NULL OR pv.value = '' OR pv.value = 'ERR', IFNULL(ucadef.value, 'FONG'), pv.value) AS CodeAnalytique
	, ucadef.value AS CodeAnalParDef
	FROM (" . $sql . ") `ts`
	JOIN `fo_contacts` `ou`
		ON `ou`.`object_id` = `ts`.`contact_id`
	LEFT JOIN fo_member_custom_property_values pv
		ON pv.member_id = `ts`.`member_id`
		AND pv.custom_property_id = ?
	LEFT JOIN fo_object_properties uv
		ON uv.rel_object_id = ou.object_id
		AND uv.name = ?
	LEFT JOIN fo_object_properties ucadef /* code analytique par défaut de l'utilisateur */
		ON ucadef.rel_object_id = ou.object_id
		AND ucadef.name = ?
	...
	, DATE_FORMAT((`ts`.`start_time`),'%Y-%m')
	, `ou`.`first_name`
	, IF(pv.value IS NULL OR pv.value = '' OR pv.value = 'ERR', IFNULL(ucadef.value, 'FONG'), pv.value)
	, ucadef.value
	...
	$custom_property_id = 'Taux horaire'; //Nom de la propriété TauxHoraire ajoutée
	$params[] = $custom_property_id;

	$custom_property_id = 'Code analytique'; //Nom de la propriété CodeAnalytique par défaut ajoutée
	$params[] = $custom_property_id;
