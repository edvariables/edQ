...
function parse_date4d ($input){
	$reg_datetime = '/^(?P<d>\d{2})[-\/](?P<m>\d{2})[-\/](?P<y>\d{2})(\s(?P<h>\d{2}):(?P<i>\d{2}):(?P<s>\d{2}))?$/';
	$is_match = preg_match_all($reg_datetime, $input, $match);
	if($is_match){
		if($match['m'][0] == '00' || $match['d'][0] == '00')
			return null;
		return ( '20' . $match['y'][0] . '-' . $match['m'][0] . '-' . $match['d'][0]);
	}
	if(substr($input, 0, 10) == '0000-00-00' || $input == 'NULL')
			return null;
...
function read_file_row(&$handle, &$columns, &$separator, &$merge = null){
	...
	for($col = 0; $col < count($columns); $col++){
		...
		case 'datetime': 
			$merge[] = parse_date4d($row[$col]);
			break;
		case 'boolean': 
			$merge[] = parse_boolean4d($row[$col]);
			break;
		case 'integer': 
			$merge[] = parse_integer4d($row[$col]);
			break;
		case 'decimal': 
			$merge[] = parse_decimal4d($row[$col]);
			break;
		default:
			$merge[] = $row[$col];
			break;
