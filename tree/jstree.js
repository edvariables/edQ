 ...
 */
	...
	$.jstree.core.prototype = {
		...
		refresh_node : function (obj) {
			...
			var selected = this._data.core.selected.length ? this._data.core.selected[0] : []; //ED140524
			var opened = [], s = this._data.core.selected.concat([]);
			if(obj.state.opened === true || obj.children.length === 0) { opened.push(obj.id); } //ED140524
			...
			this._load_nodes(opened, $.proxy(function (nodes) {
				...
				this.select_node(this._data.core.selected.length == 0 ? selected : this._data.core.selected); //ED140524
				/**
		...
		/**
		 * refreshes a node in the tree (reload its children) all opened nodes inside that node are reloaded with calls to `load_node`.
		 * @name reload_node(obj)
		 * @param {mixed} obj the node
		 * @trigger refresh.jstree
		 */
		reload_node : function (obj) {
			obj = this.get_node(obj);
			if(!obj || obj.id === '#') { return false; }
			this.refresh_node(this.get_parent(obj));
			
			//obj = this.get_node(arguments[0]);
		 ...
		 * @param  {Boolean} add toServer option at trigger call. Means it's not necessary to send data to server. (ED140507)
		 * @return {Boolean}
		 * @trigger set_text.jstree
		 */
		set_text : function (obj, val, toServer) {
			var t1, t2, dom, tmp;
			if($.isArray(obj)) {
				obj = obj.slice();
				for(t1 = 0, t2 = obj.length; t1 < t2; t1++) {
			...
			if(dom.length) {
				...
				dom.empty().html(val).prepend(tmp);
				/**
		 ...
		 */
		rename_node : function (obj, val, toServer) {
			var t1, t2, old;
			if($.isArray(obj)) {
				obj = obj.slice();
				for(t1 = 0, t2 = obj.length; t1 < t2; t1++) {
					this.rename_node(obj[t1], val, toServer);
				}
				return true;
			}
			obj = this.get_node(obj);
			...
			this.set_text(obj, val, toServer); // .apply(this, Array.prototype.slice.call(arguments))
			/**
			 ...
			 */
			this.trigger('rename_node', { "node" : obj, "text" : val, "old" : old, "toServer" : toServer });
			return true;
		},
		/**
		 * set property values of a node
		 * @name rename_node(obj, properties)
		 * @param  {mixed} obj the node, you can pass an array to rename multiple nodes to the same name
		 * @param  {jsObject} properties the new text value
		 * @return {Boolean}
		 * @trigger rename_node.jstree
		 */
		update_node : function (obj, properties) {
			var t1, t2, old;
			if($.isArray(obj)) {
				obj = obj.slice();
				for(t1 = 0, t2 = obj.length; t1 < t2; t1++) {
					this.update_node(obj[t1], val);
				}
				return true;
			}
			obj = this.get_node(obj);
			if(!obj || obj.id === '#') { return false; }
			old = obj.text;
			if( !this.check("update_node", obj, this.get_parent(obj), properties) ) {
				this.settings.core.error.call(this, this._data.core.last_error);
				return false;
			}
			if( properties.text && properties.text != obj.text){
				this.set_text(obj, properties.text); // .apply(this, Array.prototype.slice.call(arguments))
			 ...
			 * @name rename_node.jstree
			 * @param {Object} node
			 * @param {Object} properties the new values
			 */
			this.trigger('update_node', { "node" : obj, "properties" : properties });
			return true;
		},
		/**
		 * remove a node
		 ...
		 * @param  {String} default_text the text to populate the input with (if omitted the node text value is used)
		 */
		edit_node : function (obj, default_text) {
			obj = this._open_to(obj);
			if(!obj || !obj.length) { return false; }
			...
			var rtl = this._data.core.rtl,
				...
				*/
				node = this.get_node(obj),
				t  = typeof default_text === 'string' ? default_text : this.get_text(obj),
				path  = this.get_path(node, ' > ', false) + ' > ' + t,
				icon  = node.icon,
				typ  = node.type,
				ext  = node.ext,
				color  = node.color,
			...
			for(var item in items){
					$icon.append($option = $("<"+"option/>", {
						...
						, "html" : '<i class="jstree-icon ' + items[item] + '"></i>'
							+ items[item].replace(/^(jstree|file\sfile)-/, "")
					}));
			...
			h1
				...
				.append($("<table>")
					...
					.append($("<tr>")
						...
						.append($("<td>")
							.html($ulvl)
							.append($('<label>utilisateur : </label>')
								.css("margin-left", "1em")
							)
							.append($user.attr({
								"size" : "3" })
		...
		items : function (o, cb) { // Could be an object directly
			return {
				...
				"edit" : {
					"separator_before"	: false,
					"separator_after"	: false,
					"_disabled"			: false, //(this.check("edit_node", data.reference, this.get_parent(data.reference), "")),
					"label"				: "Modifier",
					/*
					"shortcut"			: 114,
					"shortcut_label"	: 'CTRL+F2',
					"icon"				: "glyphicon glyphicon-leaf",
					*/
					"action"			: function (data) {
						var inst = $.jstree.reference(data.reference),
 ...
 */
	...
	(function ($, undefined) {
		$.vakata.storage = {
			...
			set : function (key, val) { return window.localStorage.setItem('jstree-' + key, val); },
			get : function (key) { return window.localStorage.getItem('jstree-' + key); },
			del : function (key) { return window.localStorage.removeItem('jstree-' + key); }
		};
	}(jQuery));
			...
			this.element
				.on('model.jstree', $.proxy(function (e, data) {
						...
						for(i = 0, j = dpc.length; i < j; i++) {
							...
							m[dpc[i]].type = c;
							if((m[dpc[i]].icon === true || m[dpc[i]].icon === null) //ED140507
							&& t[c].icon !== undefined) {
