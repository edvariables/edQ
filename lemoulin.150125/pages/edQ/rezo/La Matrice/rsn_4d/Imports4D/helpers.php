...
function get_field_value($str, $column, $charset = 'UTF-8')
{
    if(substr($str, 0, 1) == '"'
	...
	default:
		return get_utf8_string($str, $charset);
	}
}
...
function get_field_name($str, $charset = 'UTF-8')
{
	...
	return strtolower(get_utf8_string($str, $charset));
}


function get_utf8_string($str, $charset = 'UTF-8')
{
    if($charset != 'UTF-8')
...
/* get_columns
 * $columns indexes must be UTF-8
 */
function get_columns(&$columns, &$handle, &$separator, $charset = 'UTF-8'){
	$buffer = fgets($handle);
	...
	if(is_array($columns))
		foreach($columns as $col_name => $column)
			$columns[get_field_name($col_name)] = $column;
	
	//echo('<pre>helpers get_columns : '); var_dump($columns); echo '</pre>';
	...
	//echo('<pre>helpers get_columns, return : '); var_dump($cols); echo '</pre>';
	return $cols;
}

/* read_file_row
*/
function read_file_row(&$handle, &$columns, &$separator, &$merge = null, $charset = 'UTF-8'){
	$buffer = fgets($handle);
	if($buffer === FALSE) return $buffer;
	...
	for($col = 0; $col < count($columns); $col++){
		...
		$merge[] = get_field_value($row[$col], $columns[$col], $charset);
	}
	return $merge;
}
